...
// Data Cadangan (Pasti jalan walau database offline)
const backupVideos = [
  {
    id: 1,
...
// Ambil Data & Gabung Komentar dari LocalStorage (Agar Komentar Gak Pernah Hilang)
async function loadVideos() {
  ...
  // SINKRONISASI KOMENTAR LOKAL (Pencegah Hilang Saat Refresh)
...
// Render Feed TikTok (Teks dinaikkan ke bottom-20 agar TIDAK kepotong)
function renderFeed() {
  ...
  container.innerHTML = videosData.map((item, index) => `
    <div class="swiper-slide relative w-full h-full bg-black select-none">
      ...
      <!-- BOTTOM LEFT: Teks dinaikkan ke bottom-20 biar ga kepotong menu bawah -->
      ...
      <!-- RIGHT SIDE: ACTION BUTTONS (Dinaikkan ke bottom-20) -->
      <div class="absolute right-2 bottom-20 z-20 flex flex-col items-center gap-4">
        ...
        <div class="relative mb-1">
          ...
          <button onclick="this.style.display='none'" class="absolute -bottom-1 left-1/2 -translate-x-1/2 w-4 h-4 bg-pink-500 rounded-full text-white text-[10px] font-bold flex items-center justify-center">+</button>
        </div>
...
// Tambah Komentar Permanen (Simpan ke Supabase & LocalStorage)
async function addComment(event) {
  ...
  // 1. Simpan ke LocalStorage HP biar PASTI tersimpan walau di-refresh
  ...
  if (currentItem.id) {
    supabaseClient
      ...
      .then(() => {})
      .catch(err => console.log('Supabase sync warning:', err));
  }

  // 3. Update tampilan langsung
